        ...
        <Box display="flex" flexDirection="column" flex={1}>
            {!isNicknameSet ? (
                <Box display="flex" flexDirection="column" alignItems="center" p={2}>
                    <Typography variant="h6">Introduce tu nickname</Typography>
                    <TextField
                        ...
                        margin="normal"
                    />
                    <Button onClick={handleNicknameSubmit} color="primary" variant="contained">
                        Entrar al chat
                    </Button>
                </Box>
            ) : (
                        ...
                        <List>
                            {messages.map((msg, index) => (
                                <ListItem key={index} sx={{ justifyContent: msg.nickname === nickname ? 'flex-end' : 'flex-start' }}>
                                    <ListItemText
                                        primary={`${msg.nickname}: ${msg.message}`}
                                        sx={{
                                            p: 1.5,
